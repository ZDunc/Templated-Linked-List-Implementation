...
private:
   ...
   // helper functions
   void InsertFirst(Node<T>* ptr);    // helper function -- insert first node
   Node<T>* GetNewNode(const T& d);   // dynamically allocate new node
   void DeleteNode(Node<T>* ptr);   // deallocate a node
};
